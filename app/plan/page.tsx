    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 min-w-0">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xl font-semibold truncate">{current.label}</div>
    ...
    <div className="relative min-h-[calc(100vh-0px)] overflow-x-hidden">
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-6">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[320px_1fr]">
          {/* Sidebar */}
          <aside className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 min-w-0">
            ...
            <div className="mt-5 rounded-xl border border-white/10 bg-black/40 p-3 min-w-0">
              ...
              <label className="mt-3 flex cursor-pointer items-center gap-2 rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2 text-sm text-white/75 hover:bg-white/[0.04] min-w-0">
                <FileUp size={16} className="shrink-0" />
                <span className="flex-1 min-w-0 truncate">{uploadLabel}</span>
                <input
                  type="file"
                  className="hidden"
                  multiple
          ...
          {/* Main */}
          <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:p-6 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(['plan','notes','daily','practice','ask','export'] as const).map((k) => (
                      <Button
                        ...
                        onClick={() => setTab(k)}
                        className="h-8 px-2 text-xs sm:h-9 sm:px-3 sm:text-sm capitalize"
                      >
                        {k}
                      </Button>
                    ))}
            ...
            <div className="mt-6 min-w-0">
              ...
              {result && tab === 'plan' && (
                <div className="space-y-4">
                  {result.daily_plan.map((d, i) => (
                    <div key={i} className="rounded-2xl border border-white/10 bg-black/40 p-5 min-w-0">
                      <div className="flex flex-col gap-2 md:flex-row md:items-baseline md:justify-between min-w-0">
                      ...
                      <ul className="mt-3 space-y-2 text-sm text-white/75">
                        {d.tasks.map((t, idx) => (
                          <li key={idx} className="flex gap-2 min-w-0">
                            ...
                            <span className="flex-1 min-w-0 break-words"><InlineMath content={t} /></span>
                          </li>
                        ))}
                      </ul>
              ...
              {result && tab === 'notes' && (
                <div className="rounded-2xl border border-white/10 bg-black/40 p-5 max-h-[62vh] overflow-auto min-w-0">
                    ...
                    <div className="mt-6">
                      ...
                      <div className="mt-3 grid gap-3 md:grid-cols-2">
                        {result.flashcards.slice(0, 10).map((c, i) => (
                          <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 min-w-0">
              ...
              {result && tab === 'daily' && (
                <div className="grid gap-6 lg:grid-cols-[1fr_340px] min-w-0">
                  <div className="rounded-2xl border border-white/10 bg-black/40 p-5 min-w-0">
              ...
              {result && tab === 'practice' && (
                <div className="rounded-2xl border border-white/10 bg-black/40 p-5 max-h-[68vh] overflow-auto min-w-0">
                  ...
                  <div className="mt-3 space-y-5">
                    {result.practice_questions.slice(0, 20).map((q, i) => (
                      <div key={q.id || i} className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 min-w-0">
                          ...
                          <ul className="mt-2 space-y-1 text-sm text-white/75">
                            {q.options.map((o, idx) => (
                              <li key={idx} className="flex gap-2 min-w-0">
                                ...
                                <span className="flex-1 min-w-0 break-words"><MarkdownMath content={o} /></span>
                              </li>
                            ))}
                          </ul>
                        ) : null}
                        <div className="mt-3 text-sm text-white/70 min-w-0">
                          <span className="text-white/50">Answer:</span>
                          <div className="mt-1 break-words"><MarkdownMath content={q.answer || "—"} /></div>
                        </div>
                        {q.explanation ? (
                          <div className="mt-2 text-xs text-white/55 break-words"><MarkdownMath content={q.explanation} /></div>
                        ) : null}
                      </div>
                    ))}
                  </div>
                ...
                <div className="grid gap-6 lg:grid-cols-[360px_1fr] min-w-0">
                  <div className="rounded-2xl border border-white/10 bg-black/40 p-5 min-w-0">
                    ...
                    <div className="mt-4 flex items-center gap-2 min-w-0">
                      <label className="text-xs text-white/60 shrink-0">Language</label>
                      <select
                        value={askLang}
                        onChange={(e) => setAskLang(e.target.value as 'hu' | 'en')}
                        className="min-w-0 w-full sm:w-auto rounded-lg border border-white/10 bg-black/40 px-3 py-2 text-xs text-white/80"
                      >
                    ...
                    <Textarea
                      className="mt-3 min-h-[160px] w-full"
                    ...
                    {askError && <div className="mt-3 text-sm text-red-300 break-words">{askError}</div>}
              ...
              {result && tab === 'export' && (
                <div className="rounded-2xl border border-white/10 bg-black/40 p-5 space-y-4 min-w-0">
                  <div className="text-sm text-white/70 break-words">
                    Export your notes as a PDF for offline study.
                  </div>

                  {/* Export controls: wrap on mobile */}
                  <div className="flex flex-wrap gap-2">
                    <Button
                      onClick={async () => {
                        try {
                  ...
                  {result.notes?.length ? (
                    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 min-w-0">
                      ...
                      <ul className="mt-2 space-y-1 text-sm text-white/70">
                        {result.notes.slice(0, 8).map((n, i) => <li key={i} className="break-words">• {n}</li>)}
                      </ul>
                    </div>
                  ) : null}
                </div>
