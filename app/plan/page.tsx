...
function Pomodoro({ blocks }: { blocks: Block[] }) {
  ...
  return (
    <div className="space-y-4 minw-0">
      <div className="flex items-center justify-between gap-4 minw-0">
        ...
        <div className="text-right shrink-0">
          ...
          <div className="mt-1 font-mono tabular-nums text-3xl leading-none">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="ghost" onClick={() => setIdx((i) => (i + 1) % seq.length)} className="gap-2">
          Next
        </Button>
      </div>
...
export default function PlanPage() {
  ...
  async function runPlan() {
    ...
    try {
      ...
      const item: SavedPlan = { id: nowId(), createdAt: Date.now(), prompt, result: json }
  ...
  async function genAudio() {
    setAskError(null)
    const lim = canUse('audio')
    if (!lim.ok) {
      setAskError(`Free audio limit reached. Resets in ${formatReset(lim.resetAt)}.`)
      return
    }
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6 minw-0">
      {/* Top bar */}
      <div className="flex items-center justify-between gap-4 minw-0">
        <Link href="/" className="flex items-center gap-3 minw-0">
          <Image src="/assets/logo.png" alt="Examly" width={34} height={34} className="shrink-0" />
          <span className="text-base font-semibold tracking-tight truncate">Examly</span>
        </Link>
        <Link href="/" className="text-sm text-white/70 hover:text-white inline-flex items-center gap-2 shrink-0">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[320px_1fr]">
        {/* Sidebar */}
        <aside className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 minw-0">
          <div className="text-xs uppercase tracking-[0.18em] text-white/55">History</div>
          <div className="mt-3 space-y-2 max-h-[58vh] overflow-auto pr-1 minw-0">
            {history.length === 0 && <div className="text-sm text-white/55">No saved plans yet.</div>}
            {history.map((h) => (
              <button
                key={h.id}
                onClick={() => loadSaved(h.id)}
                className={
                  'w-full text-left rounded-xl px-3 py-2 border minw-0 ' +
                  (activeId === h.id
          ...
          <div className="mt-4 flex gap-2 minw-0">
            <Button
              className="gap-2 flex-1 minw-0"
              onClick={() => {
                setResult(null)
                setActiveId(null)
                setPrompt('')
          ...
          <div className="mt-5 rounded-xl border border-white/10 bg-black/40 p-3 minw-0">
            <div className="text-xs uppercase tracking-[0.18em] text-white/55">Input</div>

            <Textarea
              className="mt-2 min-h-[120px] minw-0"
              placeholder="Describe your exam: topic, date, what matters most. (Hungarian is OK)"
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
            />

            <label className="mt-3 flex cursor-pointer items-center gap-2 rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2 text-sm text-white/75 hover:bg-white/[0.04] minw-0">
              <FileUp size={16} className="shrink-0" />
              <span className="flex-1 minw-0 truncate">{uploadLabel}</span>
              <input
                type="file"
                className="hidden"
                multiple
            ...
            {hasUploads && (
              <div className="mt-2 text-xs text-white/55 minw-0 truncate">
                {files.map((f) => f.name).slice(0, 4).join(', ')}
                {files.length > 4 ? '…' : ''}
              </div>
            )}
        ...
        {/* Main */}
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:p-6 minw-0">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between minw-0">
            <div className="minw-0">
              <div className="text-xs uppercase tracking-[0.18em] text-white/55">Plan</div>
              <div className="mt-1 text-2xl font-semibold tracking-tight break-words">
                {result?.title || 'Create a plan'}
              </div>
              {result?.quick_summary && (
                <div className="mt-2 text-sm text-white/65 max-w-[70ch] break-words">
                  {result.quick_summary}
                </div>
              )}
            </div>

            {/* Tabs: mobile-safe scroller (NO page overflow) */}
          ...
          <div className="mt-6 minw-0">
            ...
            {result && tab === 'plan' && (
              <div className="space-y-4 minw-0">
                {result.daily_plan.map((d, i) => (
                  <div key={i} className="rounded-2xl border border-white/10 bg-black/40 p-5 minw-0">
                    <div className="flex flex-col gap-2 md:flex-row md:items-baseline md:justify-between minw-0">
                      <div className="text-lg font-semibold break-words">{d.day}</div>
                      <div className="text-sm text-white/60 break-words">
                        <InlineMath content={d.focus} /> • {d.minutes} min
                      </div>
                    </div>
                    <ul className="mt-3 space-y-2 text-sm text-white/75 minw-0">
                      {d.tasks.map((t, idx) => (
                        <li key={idx} className="flex gap-2 minw-0">
                          <span className="mt-[6px] h-1.5 w-1.5 rounded-full bg-white/70 shrink-0" />
                          <span className="flex-1 minw-0">
                            <InlineMath content={t} />
                          </span>
                        </li>
                      ))}
                    </ul>
                    ...
                    {Array.isArray(d.blocks) && d.blocks.length > 0 && (
                      <div className="mt-4 text-xs text-white/55 flex flex-wrap gap-2">
                        {d.blocks.map((b, j) => (
                          <span
                            key={j}
                            className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.03] px-2 py-1"
                          >
            ...
            {result && tab === 'notes' && (
              <div className="rounded-2xl border border-white/10 bg-black/40 p-5 max-h-[62vh] overflow-auto minw-0">
                ...
                {result.flashcards?.length ? (
                  <div className="mt-6 minw-0">
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-2 minw-0">
                      {result.flashcards.slice(0, 10).map((c, i) => (
                        <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 minw-0">
                          <div className="text-sm font-semibold minw-0">
                            <MarkdownMath content={String(c.front ?? '')} />
                          </div>
                          <div className="mt-2 text-sm text-white/70 minw-0">
            ...
            {result && tab === 'daily' && (
              <div className="grid gap-6 lg:grid-cols-[1fr_340px] minw-0">
                <div className="rounded-2xl border border-white/10 bg-black/40 p-5 minw-0">
                  ...
                  <div className="mt-2 text-lg font-semibold break-words">
                    <InlineMath content={result.daily_plan?.[0]?.focus || 'Focus session'} />
                  </div>
                  <ul className="mt-3 space-y-2 text-sm text-white/75 minw-0">
                    {(result.daily_plan?.[0]?.tasks ?? []).map((t, i) => (
                      <li key={i} className="flex gap-2 minw-0">
                        <span className="mt-[6px] h-1.5 w-1.5 rounded-full bg-white/70 shrink-0" />
                        <span className="flex-1 minw-0">
                          <InlineMath content={t} />
                        </span>
            ...
            {result && tab === 'practice' && (
              <div className="rounded-2xl border border-white/10 bg-black/40 p-5 max-h-[68vh] overflow-auto minw-0">
                <div className="text-xs uppercase tracking-[0.18em] text-white/55">Practice questions</div>
                <div className="mt-3 space-y-5 minw-0">
                  {result.practice_questions.slice(0, 20).map((q, i) => (
                    <div key={q.id || i} className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 minw-0">
                      <div className="text-sm font-semibold">{i + 1}.</div>
                      <div className="mt-1 minw-0">
                        <MarkdownMath content={q.question} />
                      </div>
                      {q.type === 'mcq' && q.options?.length ? (
                        <ul className="mt-2 space-y-1 text-sm text-white/75 minw-0">
                          {q.options.map((o, idx) => (
                            <li key={idx} className="flex gap-2 minw-0">
                      ...
                      <div className="mt-3 text-sm text-white/70 minw-0">
                        <span className="text-white/50">Answer:</span>
                        <div className="mt-1 minw-0">
                          <MarkdownMath content={q.answer || '—'} />
                        </div>
                      </div>
                      {q.explanation ? (
                        <div className="mt-2 text-xs text-white/55 minw-0">
            ...
            {tab === 'ask' && (
              <div className="grid gap-6 lg:grid-cols-[360px_1fr] minw-0">
                <div className="rounded-2xl border border-white/10 bg-black/40 p-5 minw-0">
                  ...
                  <Textarea
                    className="mt-3 min-h-[160px] minw-0"
                    placeholder={askLang === 'hu' ? 'Írd ide a kérdésed…' : 'Type your question…'}
                    value={askQ}
                  ...
                  <div className="mt-3 text-xs text-white/50">Free: 2 audio / 48h. Admin: unlimited.</div>
            ...
            {result && tab === 'export' && (
              <div className="rounded-2xl border border-white/10 bg-black/40 p-5 space-y-4 minw-0">
                <div className="text-sm text-white/70">Export your notes as a PDF for offline study.</div>
                ...
                {result.notes?.length ? (
                  <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 minw-0">
                    <div className="text-xs uppercase tracking-[0.18em] text-white/55">Notes</div>
                    <ul className="mt-2 space-y-1 text-sm text-white/70 minw-0">
